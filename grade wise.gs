...
function sendAbsenteeAlert() {
  ...
  Logger.log(`Grade wise student counts: ${JSON.stringify(gradeCounts)}`);
...
function generateAbsenteesHtml(rowsHtml, daysCount, totalStudents, totalAbsentees, gradeCounts) {
  var gradeSummary = Object.keys(gradeCounts).sort().map(
    g => `<span><b>${g}:</b> ${gradeCounts[g]}</span>`
  ).join('&nbsp;|&nbsp;');
...
<head>
  <style>
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f9fafb; margin: 0; padding: 25px; }
    .wrapper { max-width: 700px; margin: auto; background: #fff; border-radius: 12px; box-shadow: 0 6px 22px rgba(0,0,0,0.08); padding: 24px 32px; }
    h2 { text-align: center; color: #30a14e; margin-bottom: 16px; font-weight: 700; }
    .summary { font-size: 15px; margin-bottom: 8px; color: #222; text-align: center; }
    .grade-summary { font-size: 14px; margin-bottom: 14px; text-align: center; color: #2d7f2f; }
    table { width: 100%; border-collapse: collapse; }
    thead tr { background: #30a14e; color: #fff; font-weight: 600; }
    th, td { padding: 14px 16px; border: 1px solid #e3e6ee; text-align: left; font-size: 15px; }
    tbody tr:nth-child(odd) { background-color: #f5fafd; }
    tbody tr:hover { background-color: #d4f1dc; transition: background-color 0.3s ease; }
...
<body>
  <div class="wrapper">
    ...
    <div class="summary"><b>Total Students:</b> ${totalStudents} &nbsp;|&nbsp; <b>Total Absentees:</b> ${totalAbsentees}</div>
    <div class="grade-summary"><b>Grade-wise totals:</b> ${gradeSummary}</div>
    <table>
      <thead>
        <tr><th>Grade</th><th>Student Name</th><th>Absent Dates</th></tr>
      </thead>
