...
function generateHtml(
  ...
  const gradeRows = Object.keys(gradeCounts)
    .map((grade) => {
      ...
      const sumPresent = gradePresentPerDay[grade].reduce((a, b) => a + b, 0);
      const percent =
        gradeCounts[grade] * dateValues.length === 0 ? 0 : Math.round((sumPresent * 100) / (gradeCounts[grade] * dateValues.length));
      return `<tr><td>${grade}</td>${presentCounts}<td>${sumPresent}</td><td>${percent}%</td></tr>`;
    })
    .join("");
  ...
  const dayWisePresent = dateValues
    .map((date, i) => {
      const sumDay = Object.keys(gradeCounts).reduce((val, g) => val + gradePresentPerDay[g][i], 0);
      return `<span style="margin-right:10px"><b>${formatDateSimple(date)}:</b> ${sumDay}</span>`;
    })
    .join("");
...
<div style="text-align:center; font-weight:bold; margin-bottom:15px; font-size: 16px;">Total Absentees: ${totalAbsentees}</div>
    <table>
      <thead>
        <tr>
          <th>Grade</th>
